...
class TestJUnitXmlParser:
    def test_should_parse_single_testsuite_without_wrapper(self, tmp_path) -> None:
        p = _write(tmp_path, "results.xml", JUNIT_SINGLE_SUITE)
        entries = parse_log_file(p)
        assert len(entries) == 3

    def test_should_assign_pass_when_no_child_element(self, tmp_path) -> None:
    ...
    def test_should_assign_fail_when_failure_child_present(self, tmp_path) -> None:
        p = _write(tmp_path, "r.xml", JUNIT_SINGLE_SUITE)
        entries = parse_log_file(p)
        failing = [e for e in entries if "test_invalid_login" in e.test]
        assert failing[0].status == "FAIL"

    def test_should_assign_skip_when_skipped_child_present(self, tmp_path) -> None:
        p = _write(tmp_path, "r.xml", JUNIT_SINGLE_SUITE)
        entries = parse_log_file(p)
        skipped = [e for e in entries if "test_skip_me" in e.test]
        assert skipped[0].status == "SKIP"

    def test_should_handle_testsuites_wrapper(self, tmp_path) -> None:
    ...
    def test_should_assign_fail_when_error_child_present(self, tmp_path) -> None:
        p = _write(tmp_path, "w.xml", JUNIT_WITH_WRAPPER)
        entries = parse_log_file(p)
        errors = [e for e in entries if "error_test" in e.test]
        assert errors[0].status == "FAIL"

    def test_should_parse_time_attribute_as_float(self, tmp_path) -> None:
        p = _write(tmp_path, "r.xml", JUNIT_SINGLE_SUITE)
        entries = parse_log_file(p)
    ...
    def test_should_default_duration_to_zero_when_time_attr_missing(self, tmp_path) -> None:
        p = _write(tmp_path, "no_time.xml", JUNIT_NO_TIME_ATTR)
        entries = parse_log_file(p)
        assert len(entries) == 1
        assert entries[0].duration == 0.0

    def test_should_include_classname_in_full_name(self, tmp_path) -> None:
        p = _write(tmp_path, "r.xml", JUNIT_SINGLE_SUITE)
        entries = parse_log_file(p)
...
class TestJsonLogParser:
    def test_should_parse_plain_json_array(self, tmp_path) -> None:
    ...
    def test_should_parse_results_wrapped_json(self, tmp_path) -> None:
    ...
    def test_should_parse_tests_wrapped_json(self, tmp_path) -> None:
        data = {"tests": [{"test": "t3", "status": "PASS"}]}
        p = _write(tmp_path, "log.json", json.dumps(data))
        entries = parse_log_file(p)
        assert entries[0].test == "t3"

    def test_should_normalise_status_in_json_log(self, tmp_path) -> None:
        data = [{"test": "t", "status": "passed"}]
        p = _write(tmp_path, "log.json", json.dumps(data))
        entries = parse_log_file(p)
        assert entries[0].status == "PASS"

    def test_should_skip_invalid_entries_gracefully(self) -> None:
...
class TestPlainTextParser:
    def test_should_parse_pass_line(self, tmp_path) -> None:
    ...
    def test_should_parse_fail_line(self, tmp_path) -> None:
    ...
    def test_should_parse_success_status(self, tmp_path) -> None:
    ...
    def test_should_convert_ms_duration_to_seconds(self, tmp_path) -> None:
...
class TestFileErrors:
    def test_should_raise_file_not_found_when_path_missing(self, tmp_path) -> None:
        nonexistent = tmp_path / "ghost.xml"
        with pytest.raises(FileNotFoundError):
            parse_log_file(nonexistent)

    def test_should_raise_value_error_when_content_is_unparseable(self, tmp_path) -> None:
